...
and
[LocationAccessCheckerPlugin](src/main/java/com/google/fhir/gateway/plugin/LocationAccessCheckerPlugin.java).
The **location** plugin enforces location-hierarchy-based access (search rewrite
via `_tag`, tag validation, post-write tag persistence) and requires a Postgres
cache: when using `ACCESS_CHECKER=location`, set `ENABLE_DATASOURCE=true` and
configure a DataSource (e.g. `SPRING_DATASOURCE_URL`,
